...
ImGuiStyle::ImGuiStyle()
{
    ...
    FramePadding            = ImVec2(2,2);      // Padding within a framed rectangle (used by most widgets)
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->InnerMainRect.Min.x = window->WindowBorderSize;
        ...
        window->InnerMainRect.Max.x = window->Pos.x + window->Size.x - ImMax(window->ScrollbarSizes.x, window->WindowBorderSize) + title_bar_rect.Min.x;
